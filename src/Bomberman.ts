...
class Bomberman {
    ...
    private beginUI: ui.BomberManUI;
    ...
    constructor() {
        ...
        Laya.loader.load(
            ...
            Laya.Handler.create(this, this.resComplete),
            Laya.Handler.create(this, this.resProgress, null, false),
            Laya.Loader.ATLAS
        );
    }
    private resProgress(value) {
        if (!this.loadingUI) {
    ...
    private onKeyDown(event: Laya.Event) {
        ...
        if (this.start) {
            ...
            if (event.keyCode == 38 || event.keyCode == 40) {//up
                let items = this.beginUI.playerChoose.items;
    ...
    private onKeyUp(event: Laya.Event) {
        if (this.start) {
            if (event.keyCode == 37
                || event.keyCode == 38
                || event.keyCode == 39
